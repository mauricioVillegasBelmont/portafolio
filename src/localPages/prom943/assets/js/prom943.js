...
Pymeweb.prototype.fit_css_height = function(args){
  var self = this;
  if ((typeof args !== 'object' || !args.references || !args.target)){
    console.warn('fit_css_main misconfig');
    return;
  }
  args.references.forEach((str) => {
    var val = ($(str).outerHeight()||0)+'px';
    var _sel = str.replace('.','');
    _sel = _sel.replace('#','');
    var attr = '--pyme-'+_sel+'-height';
    $(args.target).get(0).style.setProperty(attr,val);
  });

  $(window).on('resize',function(e){
    self.fit_css_height(args);
  });
}
...
const prom955 = new Pymeweb();
$( 'document' ).ready(function() {
  prom955.init_ready(conf,triggers);
});
